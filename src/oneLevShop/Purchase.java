	...
	public static class PurchaseManager	{
		public static void processPurchase(Customer customer, Product product) {
			...
			if (isEnoughMoney) {
				if (hasPermission) {
					if (!isExpired) {
						if (isAnyProducts) {
							...
							try {
								throw new ProductOutOfStock(customer.getName() + " can not buy: " + product.getName().toString() + ".\nProduct is out of stock!");
						...
						try {
							throw new ProductHasExpired(customer.getName() + " can not buy: " + product.getName().toString() + ".\nProduct has expired!");
					...
					try {
						throw new DontHavePermissionToBuy(customer.getName() + " can not buy: " + product.getName().toString() + ".\n" + customer.getName() + " dont have permission!");
				...
				try {
					throw new NotEnoughMoneyInBalance(customer.getName() + " can not buy: " + product.getName().toString() + ".\n" + customer.getName() + " dont have enough money!");
		...
		public static boolean permisionChecker(Customer customer, Product product) {
			boolean hasPermission = true;
			
			switch (product.getAgeRestriction().toString()) {
			case "NONE": hasPermission = true;
				break;
