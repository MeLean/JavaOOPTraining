...
public class FoodProduct extends Product implements Expirable {
	...
	public void setExpireDate(String expireDate) {
		...
		try {
			...
			System.out.println("Expire date must be in format: " +  DATE_FORMATER.toPattern());
			System.out.println("Please, enter date in correct format or close application!");
	...
	public BigDecimal getPrice() {
		Date today = new Date();
		// to convert days to milliseconds we must multiply days, 1000 (milliseconds), 60 (seconds), 60 (minutes), 24 (hours)
		Date discountDate = new Date(this.expireDate.getTime() - (1000 * 60 * 60 * 24 * DISCOUNT_DAYS_BEFOR_EXPIRE));
	...
	public String toString() {
		return super.toString() 
				+ String.format(", expire date: %s", DATE_FORMATER.format(expireDate));
	}
